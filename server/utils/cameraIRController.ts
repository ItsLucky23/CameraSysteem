// HARDWARE IR SENSOR DELEGATION (start) — file is dormant; the IR LED ring's
// onboard CdS photoresistor handles auto on/off in hardware, so we don't drive
// brightness from software. To revert (use software auto-IR + IMX708 luminance
// instead), uncomment the import block, the constants/helpers block, and the
// onThumbnailUpdated body below — and revert the matching markers in
// raspberry_pi_adapter.py, mock_adapter.py, and src/cameras/page.tsx.
...
// HARDWARE IR SENSOR DELEGATION (start) — constants + helpers commented out
...
export const onThumbnailUpdated = async ({
  cameraId: _cameraId,
  jpegBase64: _jpegBase64,
}: {
  cameraId: string;
  jpegBase64: string;
}): Promise<void> => {
  // HARDWARE IR SENSOR DELEGATION (start)
  // The IR ring has its own onboard CdS photoresistor that gates the LEDs
  // based on ambient light, completely in hardware. Our software auto-IR was
  // fighting it. Function kept exported so setIRMode_v1's immediate-eval
  // call still has a target — it just no-ops. To revert, uncomment the
  // body below and the imports/helpers blocks above.
  return;
  // HARDWARE IR SENSOR DELEGATION (end)
